...
for(const file of centerCss){
  if(!fs.existsSync(path.join(root,file)))fail(`研究センターCSSなし: ${file}`);
}
const registry=read('beta3-center-overrides.css');
for(const marker of ['beta3-center-overrides-base-r20260902-01.css','beta3-center-approved-graphics.css']){
  if(!registry.includes(marker))fail(`Beta3 CSS registryのimport欠落: ${marker}`);
}
ok('Beta3研究センターCSS registry＋base＋approved graphics');

// 27遺産すべてに承認済みグラフィックが割り当てられ、実ファイルが存在することを確認する。
const approvedCss=read('beta3-center-approved-graphics.css');
for(const site of content.sites){
  const selector=`body.site-${site.slug} .obj.display:after`;
  if(!approvedCss.includes(selector))fail(`承認済み展示グラフィックのCSS割当なし: ${site.slug}`);
}
const approvedUrls=[...approvedCss.matchAll(/url\(['"]?(docs\/assets\/heritage-graphics\/approved\/[^)'"\s]+)['"]?\)/g)].map(m=>m[1]);
if(new Set(approvedUrls).size!==27)fail(`承認済み展示グラフィックは27ファイル必要です: ${new Set(approvedUrls).size}`);
for(const file of approvedUrls){
  if(!fs.existsSync(path.join(root,file)))fail(`承認済み展示グラフィックなし: ${file}`);
}
ok('承認済み展示グラフィック 27件');
