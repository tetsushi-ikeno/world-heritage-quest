...
理由: これらはBeta3の正式実行経路に含まれる一方、ファイル名からは旧版・試作に見えるため、先祖返りや誤削除を誘発する。
...
### 4. 研究センターCSSが積層
`research-center-beta3.html` はBeta2テーマ3枚＋Beta3 overrideを順番に読み込む。CSSの優先順位変更による先祖返りリスクがある。

### 5. 自動チェックが現行Beta3を十分監視していない
`.github/workflows/beta-check.yml` と `scripts/validate-beta.mjs` はBeta2/旧ファイル中心で、`beta3.html`、`research-center-beta3.html` 等が直接の構文検証対象になっていない。
