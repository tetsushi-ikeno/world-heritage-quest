...
mainの `.github/workflows/beta-check.yml` と `scripts/validate-beta.mjs` は旧Beta中心。この整理でBeta3直接検証へ更新する。

### 6. 画像収集Actionsが通常PRでmainへ書き込む危険があった
`collect-heritage-images.yml` と `collect-branch-site-images.yml` はPRを契機に起動し、mainをcheckoutして生成画像をmainへcommitする構成だった。2026-09-02にmain側で `workflow_dispatch` の手動実行専用へ修正済み。

## 次の作業

1. Beta3ファイルを自動チェック対象へ追加する。
2. UNRESOLVEDを参照検索して分類する。
3. Loader/hotfix/CSS積層を、機能を変えずに正式ファイルへ段階統合する。
4. 回帰テスト後にLEGACY CANDIDATEを削除する。
5. 最後に `index.html` を正式なBeta3入口へ統一する。

削除は上記1〜4の安全確認が完了するまで行わない。
