...
function updateProgress(state){
 let panel=$('phase71Progress');
 if(!panel){
  panel=document.createElement('button');panel.id='phase71Progress';panel.className='phase71Progress';panel.type='button';panel.innerHTML='<div class="phase71ProgressTitle">世界遺産の書</div><div id="phase71ProgressBody"></div><div id="phase71ProgressHint" class="phase71ProgressHint"></div>';document.querySelector('.recordSection')?.appendChild(panel);panel.onclick=()=>{if(E.getState().screen==='site')E.dispatch({type:'OPEN_CODEX'});};
 }
 $('progressHud')?.classList.add('phaseHidden');
 const body=$('phase71ProgressBody'),hint=$('phase71ProgressHint');if(!body||!hint)return;
 if(state.screen==='site'&&state.siteId){
  const config=C.sites[state.siteId],cards=state.progress.siteCards[state.siteId],count=E.acquiredCount(state.siteId);
  body.innerHTML=`<div class="phase71SiteProgressName">${config.shortName}</div><div class="phase71Meter"><i style="width:${Math.round(count/config.cards.length*100)}%"></i></div><div class="phase71Count">${count} / ${config.cards.length} 記録</div><div class="phase71MiniList">${config.cards.map(c=>`<span class="${cards[c.id]?'done':'missing'}">${cards[c.id]?'✓':'?'} ${c.property}</span>`).join('')}</div>`;
  hint.textContent='タップして図鑑をひらく';panel.classList.add('canOpen');return;
 }
 body.innerHTML=`<div class="phase71AreaRow"><span>知床</span><b>${state.progress.siteCleared.shiretoko?'✓ CLEAR':`${E.acquiredCount('shiretoko')}/8`}</b></div><div class="phase71AreaRow"><span>縄文遺跡群</span><b>${state.progress.siteCleared.jomon?'✓ CLEAR':`${E.acquiredCount('jomon')}/8`}</b></div>`;
 if(!state.progress.criteriaBranchCleared)body.insertAdjacentHTML('afterbegin','<div class="phase71AreaRow phase72BranchProgress"><span>登録基準支部</span><b>まずここへ</b></div>');
 else body.insertAdjacentHTML('afterbegin','<div class="phase71AreaRow phase72BranchProgress"><span>登録基準支部</span><b>✓ CLEAR</b></div>');
 hint.textContent='北海道エリアの調査状況';panel.classList.remove('canOpen');
}
...
function renderStory(state,ui){
 ...
 ui.story.innerHTML=`<div class="phase71OrientationHead"><div class="phase71MiniPiramiton"><div class="pyramidBody"></div><div class="pyramidFace"></div><div class="pyramidMouth"></div></div><div><div class="phase71StoryKicker">世界遺産研究センター</div><h2>本部オリエンテーション</h2></div></div><div class="phase71Speech"><strong>ピラミトン：</strong> 世界遺産には、「なぜ世界的に大切なのか」を判断するための<strong>登録基準が10個</strong>あるよ。<br><br>今は覚えなくて大丈夫。北海道にある研究センターの支部で、実際の世界遺産と結びつけながら少しずつ知っていこう！</div><div class="phase72TenCriteriaHint">${C.criteria.map(c=>`<span>${c.n}</span>`).join('')}</div><div class="phase71StoryActions"><button class="bigbtn" id="phase8StartJourney">北海道へ行ってみよう</button></div>`;
 $('phase8StartJourney').onclick=()=>E.dispatch({type:'START_JOURNEY'});
}
function branchHead(){return `<div class="phase72BranchHead"><div class="phase71MiniPiramiton"><div class="pyramidBody"></div><div class="pyramidFace"></div><div class="pyramidMouth"></div></div><div><div class="phase71StoryKicker">世界遺産研究センター</div><h2>北海道・登録基準支部</h2></div></div>`;}
function renderBranch(state,ui){
 const b=state.branch;ui.branch.classList.remove('hidden');
 if(b.step===0){
  ui.branch.innerHTML=branchHead()+`<div class="phase72Step">1 / 3　正式表記を読めるようになろう</div><h3>登録基準はローマ数字で書かれる</h3><div class="phase72RomanKeys"><div><b>I</b><span>= 1</span></div><div><b>V</b><span>= 5</span></div><div><b>X</b><span>= 10</span></div></div><div class="phase72RuleGrid"><div><strong>右側の I は足す</strong><span>VI = 5 + 1 = <b>6</b></span><span>VII = 7　／　VIII = 8</span></div><div><strong>左側の I は1引く</strong><span>IV = 5 - 1 = <b>4</b></span><span>IX = 10 - 1 = <b>9</b></span></div></div><div class="phase72Tip">I・V・X の3つと、4と9の「1引く」ルールが分かれば、登録基準の (i)〜(x) は全部読めるよ。</div><div class="phase72Actions"><button class="bigbtn" id="phase8BranchBack">いったん戻る</button><button class="bigbtn" id="phase8BranchNext">10個の基準を見る</button></div>`;
  $('phase8BranchBack').onclick=()=>E.dispatch({type:'CLOSE_OVERLAY'});$('phase8BranchNext').onclick=()=>E.dispatch({type:'BRANCH_NEXT'});return;
 }
 if(b.step===1){
  ui.branch.innerHTML=branchHead()+`<div class="phase72Step">2 / 3　10個の基準を見渡そう</div><h3>数字・意味・正式表記をセットにする</h3><div class="phase72IconNote">マークはゲームで覚えやすくするための目印だよ。</div><div class="phase72CriteriaGrid">${C.criteria.map(c=>`<div class="phase72Criterion"><span class="phase72CriterionIcon">${c.icon}</span><span class="phase72CriterionNum">${c.n}</span><span class="phase72CriterionText"><b>${c.title}</b><small>${c.note}</small></span><span class="phase72CriterionRoman">(${c.roman})</span></div>`).join('')}</div><div class="phase72Actions"><button class="bigbtn" id="phase8BranchPrev">もどる</button><button class="bigbtn" id="phase8BranchNext">日本の特徴を見る</button></div>`;
  $('phase8BranchPrev').onclick=()=>E.dispatch({type:'BRANCH_PREV'});$('phase8BranchNext').onclick=()=>E.dispatch({type:'BRANCH_NEXT'});return;
 }
 if(b.step===2){
  const max=Math.max(...C.criteria.map(c=>c.count));
  ui.branch.innerHTML=branchHead()+`<div class="phase72Step">3 / 3　日本の世界遺産を数字で見よう</div><h3>日本では、どの登録基準が多い？</h3><p class="phase72StatsLead">1つの遺産に複数の基準が認められるため、件数を足すと遺産数より多くなるよ。</p><div class="phase72Bars">${C.criteria.map(c=>`<div class="phase72BarRow"><span>${c.icon} ${c.n}</span><div><i style="width:${Math.round(c.count/max*100)}%"></i></div><b>${c.count}件</b><small>${c.title}</small></div>`).join('')}</div><div class="phase72Tip"><b>見つけてみよう：</b> 日本では基準2「文化交流」や基準4「建築・科学技術」が多く、基準8「地球の歴史」は現在0件。旅をしながら、この違いがどこから生まれるのか考えてみよう。</div><div class="phase72Actions"><button class="bigbtn" id="phase8BranchPrev">もどる</button><button class="bigbtn" id="phase8BranchNext">3問だけ確認する</button></div>`;
  $('phase8BranchPrev').onclick=()=>E.dispatch({type:'BRANCH_PREV'});$('phase8BranchNext').onclick=()=>E.dispatch({type:'BRANCH_NEXT'});return;
 }
 if(b.step===3){
  const q=C.branchQuiz[b.quizIndex];
  ui.branch.innerHTML=branchHead()+`<div class="phase72Step">登録基準チェック ${b.quizIndex+1} / ${C.branchQuiz.length}</div><h3 class="phase72Question">${q.q}</h3><div class="phase72Choices">${q.choices.map((c,i)=>`<button class="bigbtn phase72Choice phase8BranchChoice" data-i="${i}" ${b.answered?'disabled':''}>${c}</button>`).join('')}</div><div id="phase72QuizFeedback" class="phase72QuizFeedback">${b.answered?`<b>${b.feedback.startsWith('正解！')?'正解！':'おしい！'}</b> ${b.feedback.replace(/^正解！ |^おしい！ /,'')}<br><button class="bigbtn" id="phase8BranchQuizNext">${b.quizIndex===C.branchQuiz.length-1?'結果を見る':'つぎの問題'}</button>`:''}</div>`;
  ui.branch.querySelectorAll('.phase8BranchChoice').forEach(btn=>btn.onclick=()=>E.dispatch({type:'BRANCH_ANSWER',index:Number(btn.dataset.i)}));$('phase8BranchQuizNext')&&($('phase8BranchQuizNext').onclick=()=>E.dispatch({type:'BRANCH_QUIZ_NEXT'}));return;
 }
 const passed=b.correct>=2;
 ui.branch.innerHTML=branchHead()+(passed?`<div class="phase72BranchClear">✓</div><h3>登録基準支部 CLEAR！</h3><p>${C.branchQuiz.length}問中${b.correct}問正解！<br>I・V・Xの読み方と、10個の登録基準を確認できた。</p><div class="phase72Tip">ここから先は、実際の世界遺産で「どの基準が、なぜ認められたのか」を調べていこう。</div><div class="phase72Actions"><button class="bigbtn" id="phase8BranchComplete">北海道の旅を続ける</button></div>`:`<h3>あと少し！</h3><p>${C.branchQuiz.length}問中${b.correct}問正解。2問正解でCLEARだよ。</p><div class="phase72Actions"><button class="bigbtn" id="phase8BranchReview">もう一度見直す</button><button class="bigbtn" id="phase8BranchRetry">もう一度挑戦する</button></div>`);
 $('phase8BranchComplete')&&($('phase8BranchComplete').onclick=()=>E.dispatch({type:'BRANCH_COMPLETE'}));$('phase8BranchReview')&&($('phase8BranchReview').onclick=()=>E.dispatch({type:'BRANCH_REVIEW'}));$('phase8BranchRetry')&&($('phase8BranchRetry').onclick=()=>E.dispatch({type:'BRANCH_RETRY'}));
}
...
function render(state){
 const ui=ensureUi();allPanelsHidden();updateProgress(state);
